...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        MoneyTransactionServlet moneyTransServlet = new MoneyTransactionServlet();
        ResultServlet resultServlet = new ResultServlet();

        ServletContextHandler context = new ServletContextHandler(ServletContextHandler.SESSIONS);
        context.addServlet(new ServletHolder(apiServlet), "/api/*");
        context.addServlet(new ServletHolder(regServlet), "/registration");
        context.addServlet(new ServletHolder(moneyTransServlet), "/transaction");
